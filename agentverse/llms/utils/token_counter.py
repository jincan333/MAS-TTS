...
def count_string_tokens(prompt: str = "", model: str = "gpt-3.5-turbo") -> int:
    if model.startswith("gpt-3.5-turbo") or model.startswith("gpt-4") or model.startswith("o3"):
        return len(tiktoken.encoding_for_model("gpt-4o").encode(prompt))
    elif model in ["deepseek-chat", "deepseek-reasoner"]:
        deepseek_tokenizer_dir = os.path.join(os.environ['MY_PROJECT'], "tokenizer")
        encoding = transformers.AutoTokenizer.from_pretrained(deepseek_tokenizer_dir, trust_remote_code=True)
