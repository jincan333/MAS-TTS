...
class TaskSolving:
    ...
    def run(self):
        ...
        self.logs = []
        current_resources = "No resources yet."
        advice = "No advice yet."
        previous_plan = "No solution yet."
        while not self.environment.is_done():
            result, advice, previous_plan, current_resources, logs, success = asyncio.run(
                self.environment.step(advice, previous_plan, current_resources)
            )
            self.logs += logs
